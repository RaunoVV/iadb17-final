...
export const WorkflowForm = (props: { action: string }) => {
	...
	const { autocompleteProps: hardwareAutoCompleteProps } =
		useAutocomplete<THardware>({
			resource: "hardware",
		});
	...
	const { autocompleteProps: templateAutoCompleteProps } =
		useAutocomplete<TTemplate>({
			resource: "template",
		});
			...
			<Controller
				...
				render={({ field, fieldState }) => (
					<Autocomplete
						...
						{...field}
						onChange={(_, value: THardware | null) =>
							field.onChange(value?.name || null)
						}
						onInputChange={() => {}}
						filterOptions={hardwareFilterOptions}
						value={
							hardwareAutoCompleteProps.options.find(
			...
			<Controller
				...
				render={({ field, fieldState }) => (
					<Autocomplete
						...
						{...field}
						onChange={(_, value: TTemplate | null) =>
							field.onChange(value?.name || null)
						}
						onInputChange={() => {}}
						filterOptions={templateFilterOptions}
						value={
							templateAutoCompleteProps.options.find(
								(option) => option.name === field.value,
							) ?? null
						}
						getOptionLabel={(item) => {

...
export const HardwareInterface = (props: InterfaceProps) => {
    ...
    return (
        <FormGroup>
            ...
            <FormControlLabel
                control={<input type="checkbox" {...register(`spec.interfaces.${props.index}.dhcp.uefi` as const)} />}
                label="UEFI"
            />

            <FormControlLabel
                control={
                    <input type="checkbox" {...register(`spec.interfaces.${props.index}.netboot.allowPXE` as const)} />
                }
                label="Allow Netboot"
            />
            <FormControlLabel
                control={
                    <input
                        type="checkbox"
                        {...register(`spec.interfaces.${props.index}.netboot.allowWorkflow` as const)}
                    />
                }
                label="Allow Workflow"
            />
            <DeleteButton key={props.index} onClick={() => props.remove(props.index)}>
                Remove
            </DeleteButton>
        </FormGroup>
    );

...
import {
	hardwareInterfaceSchema,
	type THardwareInterface,
} from "shared/types/Hardware";

export const HardwareForm = (props: {
	action: string;
	defaultInterfaceValue: THardwareInterface;
}) => {
	const {
		register,
		control,
	...
	const anotherInterfaceClickHandler = (
			...
			{fields.map((intf, index) => (
				<>
					<HardwareInterface
						...
						data={intf}
					/>
					<DeleteButton key={index} onClick={() => remove(index)}>
						Remove
					</DeleteButton>
				</>
			))}
